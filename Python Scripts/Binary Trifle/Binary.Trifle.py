...
FileContents = int(open("HiScores.txt").read())
...
def Move_Player(direction):
    ...
    if int(rowArray[1][gPlayerPos]) == 1:
        ...
        if FileContents  < gScore:
            FileWrite = open("HiScores.txt",'w')
            HiScore = str(gScore)
            FileWrite.write(HiScore)
            FileWrite.close
            HighScoreLabel.configure(text="High Score: " + str(gScore))
    else:
        gScore = gScore + int(rowArray[1][0]) + int(rowArray[1][1]) + int(rowArray[1][2]) + int(rowArray[1][3]) + int(rowArray[1][4]) + int(rowArray[1][5]) + int(rowArray[1][6])
        del rowArray[0]
